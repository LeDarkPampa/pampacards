...
export class RechercheCombatComponent implements OnInit, OnDestroy {
  ...
  startSearch() {
    this.http.post<IDeck[]>('http://localhost:8080/backend/api/addUserToFight', this.authService.userId).subscribe(data => {
    })
  ...
  stopSearch() {
    this.http.post<IDeck[]>('http://localhost:8080/backend/api/removeUserToFight', this.authService.userId).subscribe(data => {
    })
  ...
  getUsersSearchingFight() {
    this.http.get<any>('http://localhost:8080/backend/api/usersToFight').subscribe({
      next: data => {
  ...
  updateDemandeCombat(demandeCombat: IDemandeCombat) {
    this.http.post<any>('http://localhost:8080/backend/api/updateDemandeCombat', demandeCombat).subscribe({
      next: response => {
  ...
  challengeOpponent(opponent: IUtilisateur) {
    ...
    this.http.post<any>('http://localhost:8080/backend/api/createChallenge', data).subscribe({
      next: response => {
    ...
    this.zone.run(() => {
      ...
      ref.onClose.subscribe((demandeCombat: IDemandeCombat) => {
        ...
        if (demandeCombat.status === DemandeCombatStatusEnum.DEMANDE_ACCEPTEE) {
          this.http.post<any>('http://localhost:8080/backend/api/createPartie', demandeCombat).subscribe({
            next: response => {
  ...
  deleteDemandeCombat(demandeCombat: IDemandeCombat) {
    ...
    this.http.request('delete', 'http://localhost:8080/backend/api/demandeCombat', {body: demandeCombat}).subscribe({
      next: data => {
      },
      error: error => {
        console.error('There was an error!', error);
  ...
  private checkSiDejaPartieEncours() {
    this.http.get<IPartie>('http://localhost:8080/backend/api/partieEnCours?utilisateurId=' + this.userId).subscribe({
      next: partie => {
  ...
  private getAllFormats() {
    this.http.get<IFormat[]>('http://localhost:8080/backend/api/formats').subscribe({
      next: data => {
