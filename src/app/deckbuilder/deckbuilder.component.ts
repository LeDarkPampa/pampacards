...
export class DeckbuilderComponent implements OnInit {
  ...
  getAllPlayerDecks() {
    this.http.get<IDeck[]>('http://localhost:8080/backend/api/decks?userId='+this.authService.userId).subscribe({
      next: data => {
  ...
  saveDeck(deck: IDeck) {
    if (!deck.nom) {
      ...
      this.http.post<IDeck[]>('http://localhost:8080/backend/api/deck', deck).subscribe(data => {
        this.getAllPlayerDecks();
      })
    }
  }
  ...
  duplicateDeck(deck: IDeck) {
    if (!deck.nom) {
      ...
      this.http.post<IDeck[]>('http://localhost:8080/backend/api/deck', duplicatedDeck).subscribe(data => {
        this.getAllPlayerDecks();
      })
    }
  }
  ...
  delete(selectedDeck: IDeck) {
    this.http.request('delete', 'http://localhost:8080/backend/api/deck', {body: selectedDeck}).subscribe({
      next: data => {
        this.getAllPlayerDecks();
        // @ts-ignore
