...
export class PartieComponent implements OnInit, OnDestroy {
  ...
  finDeTour() {
    if (this.estJoueurActif) {
      ...
      this.http.post<any>('http://localhost:8080/backend/api/partieEvent', event).subscribe({
        next: response => {
        },
        error: error => {
  ...
  private terminerPartie() {
    ...
    this.http.post<any>('http://localhost:8080/backend/api/enregistrerResultat', event).subscribe({
      next: response => {
      },
      error: error => {
        console.error('There was an error!', error);
  ...
  private getPartie() {
    this.http.get<IPartie>('http://localhost:8080/backend/api/partie?partieId=' + this.partieId).subscribe({
      next: partie => {
        this.partie = partie;
        this.initValues();
      },
  ...
  private getEventsPartie() {
    this.http.get<IEvenementPartie[]>('http://localhost:8080/backend/api/partieEvents?partieId=' + this.partieId).subscribe({
  ...
  sendMessage() {
    if (this.message && this.message != '') {
      ...
      this.http.post<any>('http://localhost:8080/backend/api/chatMessages', message).subscribe({
        next: () => {
          this.getChatPartieMessages();
        },
        error: error => {
  ...
  private getChatPartieMessages() {
    this.http.get<IChatPartieMessage[]>('http://localhost:8080/backend/api/chatMessages?partieId=' + this.partieId).subscribe({
      next: chatMessages => {
        // @ts-ignore
